...
def generate_music(music_style_prompt: str, lyrics_text: str):
    ...
    cfm, tokenizer, muq, vae = prepare_model(max_frames, device, repo_id)

    # Taking users prompt for style
    ref_prompt = music_style_prompt #get_musicstyle_prompt(music_style_prompt)
    style_with_a_twist=get_musicstyle_prompt(ref_prompt)
    output_filename = get_filename(style_with_a_twist)

    # lyrics (tomt för instrumelta låtar)
    ##lrc = ""

    #with open("infer/example/eg_en_full.lrc", "r", encoding='utf-8') as f:
    #    lyrics_text = f.read()
    #print(lyrics_text)
...
#style = "piano violin spooky"
#text = get_lyrics_prompt("Muder mystery between AI systems", style)
#print(text)
#generate_music(style, text)
